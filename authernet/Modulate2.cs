...
public class DPSKDemodulatorX
(DPSKSymbol symbol, int maxByteLength)
{
    private readonly int numSamplesPerSymbol = symbol.Option.NumSamplesPerSymbol;

    protected readonly DPSKSymbol symbol = symbol;
    protected readonly int maxByteLength = maxByteLength;

    public bool TryRead(ref ReadOnlySequence<float> samples, out ReadOnlySequence<byte> data)
    {
    ...
    protected bool TryRead(ref ReadOnlySequence<float> samples, Span<byte> data)
    {
        ...
        {
            samples.TryReadOrCopy(out var readed, buffer);
            data[i] = ModulateHelper.DotProductDemodulateByte(readed, symbol.Samples.Span[0].Span);
        }
        return true;
    }

    protected virtual bool TryGetLength(ReadOnlySequence<float> samples, out int length)
    {
...
public class DPSKDemodulatorX<TSample, TSize>(DPSKSymbol symbol, int maxByteLength)
    : DPSKDemodulatorX(symbol, maxByteLength)
    where TSample : unmanaged, IBinaryNumber<TSample>
    where TSize : unmanaged, IBinaryInteger<TSize>
{
    protected override bool TryGetLength(ReadOnlySequence<float> samples, out int length)
    {
