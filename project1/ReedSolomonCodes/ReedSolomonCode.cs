...
public class ReedSolomonCode
{
    ...
    private readonly int[] galyaFieldPolynoms = [//  MM=3 poly x3+x+1=1011=0xB
    ...
    internal ReedSolomonCode(bool turnOverBits = true, int bitsNumberInSymbol = 8, int correctableSymbolsNumber = 8)
    {
        ...
        for (int j = 0; j < ParitySymbolsNumber; j++)
        {
            ...
            int resultDegree = PolyMul(j, gpoly1, 1, gpoly2, _generatingPolynomial);
    ...
    private int Div(int a, int b)
    {
        return ((a == 0) || (b == 0))
                   ? 0
                   : _indexToValue[(CodewordLength + _valueToIndex[a] - _valueToIndex[b]) % CodewordLength];
    }
    ...
    {
        if ((dataInput == null) || (dataInput.Length != (CodewordLength - ParitySymbolsNumber)))
            return false;
    ...
    {
        //  Parameter check
        if ((dataInput == null) || (dataInput.Length != CodewordLength))
            return null;

        int[] dataOutput = new int[CodewordLength];
        int dataLength = dataInput.Length;
        int distortionNumber = (distortionIndexes != null) && (distortionIndexes.Length <= ParitySymbolsNumber)
                                   ? distortionIndexes.Length
                                   : 0;
        ...
        {
            locatore[i] = locatorc[i] = p1[i] = p2[i] = poseInd[i] = possInd[i] = syndr[i] = syndc[i] = syndrMod[i] = 0;
        ...
        if (distortionNumber != 0 && distortionIndexes != null)
        {
            ...
            for (j = 0; j < distortionNumber; j++) //  Multiplying by the number of brackets
            {
                ...
                var rdegree = PolyMul(j, p1, 1, p2, locatorc); //  Resulting degree
            ...
            if (distortionNumber < (2 * CorrectableSymbolsNumber - 1))
            {
                ...
                for (i = 0; i < (2 * CorrectableSymbolsNumber - distortionNumber); i++)
                {
                    ...
                    {
                        syndrMod[i] = (syndrMod[i] ^ Mul(syndr[i + j], locatorc[distortionNumber - j]));
                    }
                }

                for (i = 0; i < 2 * CorrectableSymbolsNumber; i++)
                    syndr[i] = 0;
        ...
        if ((clear == 0) && (err != 0))
        {
            ...
            for (i = 0; i < err; i++)
            {
                locatore[(CorrectableSymbolsNumber - err) + i] = Div(matrSyndr[i, err], matrSyndr[i, i]);
            }
            ...
            for (i = 0; i < bufferSize; i++)
            {
                ...
                for (j = 0; j < err + 1; j++)
                {
                    ...
                    position[i] =
                        SymbolBitsMask & (position[i] ^ Mul(x, locatore[(CorrectableSymbolsNumber - err) + j]));
                }
            }
        ...
        for (i = 0; i < err; i++)
        {
            for (j = 0; j < err; j++)
            {
                matrErr[i, j] = _indexToValue[(geInd[j] * (i + ((TurnOverBits == false) ? 1 : 0))) % CodewordLength];
            }
        }
        ...
        for (k = 0; k < err; k++)
        {
            for (j = 0; j < err; j++)
            {
                ...
                if (matrErr[k, k] == 0)
                {
                    ...
                    for (tc = 0; tc < err + 1; tc++)
                    {
                        matrErr[k, tc] = (matrErr[k, tc] ^ matrErr[ts, tc]);
                    }
                }
        ...
        for (i = 0; i < err; i++)
        {
            ...
            if (_valueToIndex[error[i]] == CodewordLength)
                continue;
    ...
    {
        if ((dataInput == null) || (dataInput.Length != (CodewordLength + 1 - ParitySymbolsNumber)))
            return null;
        ...
        for (i = start; i <= CodewordLength; i++)
        {
            ...
            {
                r =
                    (((r == 0) ? 0 : (_indexToValue[(_valueToIndex[r] + idexI) % CodewordLength])) ^
                     (dataInput[indexInput] & SymbolBitsMask));
                indexInput += 1;
                l =
                    (((l == 0) ? 0 : (_indexToValue[(_valueToIndex[l] + idexI) % CodewordLength])) ^
                     (dataInput[indexInput] & SymbolBitsMask));
                indexInput += 1;
            }
    ...
    public int[]? DecodeHorner(int[] dataInput, int[] distortions)
    {
        ...
        //  Parameter check
        if ((dataInput == null) || (dataInput.Length != dataLength) || (distortions == null) ||
            (distortions.Length != dataLength))
            return null;
        ...
        for (i = 0; i < dl; i++)
        {
            dataOutput[i * 2 + 0] = Div(mas[matrixSize + i * 2 + 1], mas[i * dl + i]);
            dataOutput[i * 2 + 1] = Div(mas[matrixSize + i * 2 + 0], mas[i * dl + i]);
        }
